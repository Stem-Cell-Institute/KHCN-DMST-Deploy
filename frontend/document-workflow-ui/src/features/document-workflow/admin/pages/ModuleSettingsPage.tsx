      ...
      <Card className="p-4">
        <form
          ...
          onSubmit={async (e) => {
            ...
            const payload = {
              default_assignment_days: Number(
                (f.elements.namedItem("default_assignment_days") as HTMLInputElement).value
              ),
              default_review_remind_days: Number(
                (f.elements.namedItem("default_review_remind_days") as HTMLInputElement).value
              ),
              email_enabled: (f.elements.namedItem("email_enabled") as HTMLInputElement)
                .checked,
              step5_recipient_mode: (
                f.elements.namedItem("step5_recipient_mode") as HTMLSelectElement
              ).value,
            };
        ...
        >
          <label className="text-sm">
            Deadline mặc định (ngày)
            <Input
              name="default_assignment_days"
              type="number"
              defaultValue={settings.default_assignment_days || "14"}
            />
          </label>
          <label className="text-sm">
            Nhắc rà soát trước hiệu lực (ngày)
            <Input
              name="default_review_remind_days"
              type="number"
              defaultValue={settings.default_review_remind_days || "180"}
            />
          </label>
          <label className="flex items-center gap-2 text-sm md:col-span-2">
            <input
              name="email_enabled"
              type="checkbox"
              defaultChecked={settings.email_enabled === "1"}
            />
          ...
          <label className="text-sm md:col-span-2">
            ...
            >
              <option value="module_manager_assigned">
                Workflow Manager + Người soạn thảo được phân công
              </option>
              <option value="broad_roles">
                Toàn bộ role Drafter + Leader + Reviewer + Người soạn thảo
              </option>
            </select>
          </label>
          ...
          {types.map((t) => (
            <form
              ...
              onSubmit={async (e) => {
                ...
                await saveDocumentType({
                  ...
                  is_active: (f.elements.namedItem(`active-${t.id}`) as HTMLInputElement)
                    .checked,
                  sort_order: Number(
                    (f.elements.namedItem(`sort-${t.id}`) as HTMLInputElement).value || 0
                  ),
                });
            ...
            >
              ...
              <Input
                name={`sort-${t.id}`}
                type="number"
                defaultValue={String(t.sort_order || 0)}
              />
              <label className="flex items-center gap-2 text-sm">
                <input
                  name={`active-${t.id}`}
                  type="checkbox"
                  defaultChecked={Number(t.is_active || 1) === 1}
                />
                Active
              </label>
          ...
          <form
            ...
            onSubmit={async (e) => {
              ...
              await saveDocumentType({
                ...
                is_active: true,
                sort_order: Number(
                  (f.elements.namedItem("sort-new") as HTMLInputElement).value || 0
                ),
              });
