...
import { deleteDocument, fetchDocumentDetail, fetchMe, fetchUnits, updateDocument } from "@/lib/api";
...
export function DocumentDetailPage() {
  ...
  const canManageDocs = roles.includes("module_manager") || roles.includes("master_admin") || roles.includes("admin");
    ...
    <Page>
          ...
          {doc ? (
            <>
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                <Badge tone={doc.status === "archived" ? "success" : "default"}>{doc.status}</Badge>
                {canManageDocs ? (
                  <div className="ml-auto flex gap-2">
                    <Button variant="outline" onClick={() => setEditing(true)}>Chỉnh sửa</Button>
                    <Button variant="danger" onClick={() => setConfirmDelete(true)}>Xóa hồ sơ</Button>
                  </div>
                ) : null}
              </div>
              <div className="grid gap-3 text-sm text-slate-600 md:grid-cols-4">
                <p><b>Người đề xuất:</b> {doc.proposer_id || "—"}</p>
                <p><b>Đơn vị chủ trì:</b> {doc.assigned_unit_id || "—"}</p>
                <p><b>Người soạn thảo:</b> {doc.assigned_to_id || "—"}</p>
                <p><b>Deadline:</b> {(doc.assignment_deadline || "").slice(0, 10) || "—"}</p>
              </div>
              <div className="mt-4">
        ...
        {doc ? (
          <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
            <div className="space-y-4">
              <StepForms me={me} document={doc} units={units} onUpdated={reload} />
              <Card className="p-4">
                <h3 className="mb-3 text-sm font-semibold text-slate-700">Upload file đính kèm bổ sung</h3>
                <AttachmentUploader
                  documentId={doc.id}
            ...
            <div className="space-y-4">
              <Card className="p-4">
                <h3 className="mb-3 text-sm font-semibold text-slate-700">File đính kèm theo bước</h3>
      ...
      >
        {doc ? (
          <form
            ...
            onSubmit={async (e) => {
              ...
              await updateDocument(doc.id, {
                ...
                applicableSubjects: (f.elements.namedItem("applicableSubjects") as HTMLTextAreaElement).value,
                mainContent: (f.elements.namedItem("mainContent") as HTMLTextAreaElement).value,
                executionClause: (f.elements.namedItem("executionClause") as HTMLTextAreaElement).value,
              });
          ...
          >
            <label className="text-sm">Tiêu đề<Input name="title" defaultValue={doc.title} required /></label>
            <label className="text-sm">Loại văn bản<Select name="doc_type" defaultValue={doc.doc_type}>
            ...
            <label className="text-sm md:col-span-2">Lý do<Textarea name="reason" rows={2} defaultValue={doc.reason || ""} /></label>
            <label className="text-sm md:col-span-2">Nội dung đề xuất<Textarea name="proposalSummary" rows={2} defaultValue={doc.proposal_summary || ""} /></label>
            <label className="text-sm">Căn cứ pháp lý<Textarea name="legalBasis" rows={2} defaultValue={doc.legal_basis || ""} /></label>
            <label className="text-sm">Phạm vi<Textarea name="scope" rows={2} defaultValue={doc.scope || ""} /></label>
            <label className="text-sm">Đối tượng áp dụng<Textarea name="applicableSubjects" rows={2} defaultValue={doc.applicable_subjects || ""} /></label>
            <label className="text-sm">Điều khoản thi hành<Textarea name="executionClause" rows={2} defaultValue={doc.execution_clause || ""} /></label>
            <label className="text-sm md:col-span-2">Nội dung chính<Textarea name="mainContent" rows={3} defaultValue={doc.main_content || ""} /></label>
