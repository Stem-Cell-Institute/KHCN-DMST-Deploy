...
import { deleteDocument, fetchDocuments, fetchMe, fetchUnits, updateDocument } from "@/lib/api";
...
export function DocumentListPage() {
  ...
  const filters = useMemo(() => ({ search, step, status, unitId, page: 1 }), [search, step, status, unitId]);
    ...
    <Page>
      <div className="mx-auto max-w-7xl space-y-4">
        <Card className="p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-xl font-semibold text-slate-800">Dashboard hồ sơ văn bản nội bộ</h1>
        ...
        <Card className="p-4">
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <Select value={step} onChange={(e) => setStep(e.target.value)}>
              ...
              {Object.entries(STEP_LABELS).map(([k, v]) => <option key={k} value={k}>{`Bước ${k} - ${v}`}</option>)}
            </Select>
        ...
        <Card className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr><td className="px-3 py-4 text-slate-500" colSpan={canManageDocs ? 9 : 8}>Đang tải dữ liệu...</td></tr>
                ) : rows.length === 0 ? (
                  <tr><td className="px-3 py-4 text-slate-500" colSpan={canManageDocs ? 9 : 8}>Không có hồ sơ phù hợp.</td></tr>
                ) : (
                  rows.map((r) => (
                    <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50/70">
                      <td className="px-3 py-2">{r.id}</td>
                      <td className="px-3 py-2 font-medium text-slate-700">
                        <Link to={`/documents/${r.id}`} className="text-primary-700 hover:underline">{r.title}</Link>
                      </td>
                      <td className="px-3 py-2">{r.doc_type}</td>
                      <td className="px-3 py-2">{r.assigned_unit_id || "—"}</td>
                      <td className="px-3 py-2">
                        <Badge tone={stepBadgeTone(r.current_step) as never}>{`B${r.current_step}`}</Badge>
                      </td>
                      <td className="px-3 py-2">{r.status}</td>
                      <td className="px-3 py-2">
                        <div className="w-40 rounded-full bg-slate-200">
                          <div className="h-2 rounded-full bg-primary-600" style={{ width: `${(r.current_step / 9) * 100}%` }} />
                        </div>
                      </td>
                      ...
                      {canManageDocs ? (
                        <td className="px-3 py-2">
                          <div className="flex gap-2">
                            <Button variant="outline" onClick={() => setEditing(r)}>Sửa</Button>
                            <Button variant="danger" onClick={() => setDeleting(r)}>Xóa</Button>
                          </div>
                        </td>
                      ) : null}
                    </tr>
      ...
      >
        {editing ? (
          ...
          >
            <label className="text-sm">Tiêu đề<Input name="title" defaultValue={editing.title} required /></label>
            <label className="text-sm">Loại văn bản<Input name="doc_type" defaultValue={editing.doc_type} required /></label>
            <label className="text-sm md:col-span-2">Lý do<Textarea name="reason" rows={3} defaultValue={editing.reason || ""} /></label>
