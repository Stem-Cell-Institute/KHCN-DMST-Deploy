    ...
    <div className="space-y-4">
      <Card className="p-4">
        ...
        >
          ...
          <Input placeholder="Action" value={filters.action} onChange={(e) => setFilters((p) => ({ ...p, action: e.target.value }))} />
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-xs text-slate-500">{r.new_value || r.old_value || "-"}</td>
              </tr>
            ))}
          </tbody>
        </table>
